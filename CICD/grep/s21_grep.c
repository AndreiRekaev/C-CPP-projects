...
void grep_file_process(char *filename, FILE *file) {
  ...
  size_t len;
  if (gf.i)
    flag = REG_ICASE;  // TODO: здесь можно добавить flag = REG_NEWLINE для
                       // игнорирования пустых строк при подсчете совпадений
  ...
  for (int i = 0; i < pattern_size; i++) {
    int regresult =
        regcomp(&regex, patterns[i],
                flag);  // TODO: здесь можно добавить flag = REG_NEWLINE для
                        // игнорирования пустых строк при подсчете совпадений
    if (regresult) {
