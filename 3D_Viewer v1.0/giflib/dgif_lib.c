...
GifFileType *DGifOpenFileHandle(int FileHandle, int *Error) {
  ...
  if (GifFile == NULL) {
    if (Error != NULL) *Error = D_GIF_ERR_NOT_ENOUGH_MEM;
  ...
  if (READ(GifFile, (unsigned char *)Buf, GIF_STAMP_LEN) != GIF_STAMP_LEN) {
    if (Error != NULL) *Error = D_GIF_ERR_READ_FAILED;
    (void)fclose(f);
    free((char *)Private);
    free((char *)GifFile);
    return NULL;
  ...
  if (strncmp(GIF_STAMP, Buf, GIF_VERSION_POS) != 0) {
    if (Error != NULL) *Error = D_GIF_ERR_NOT_GIF_FILE;
    (void)fclose(f);
    free((char *)Private);
    free((char *)GifFile);
    return NULL;
...
int DGifGetImageDesc(GifFileType *GifFile) {
  ...
  if (GifFile->SavedImages) {
    if ((GifFile->SavedImages = (SavedImage *)realloc(
             GifFile->SavedImages,
    ...
    if ((GifFile->SavedImages = (SavedImage *)malloc(sizeof(SavedImage))) ==
        NULL) {
      GifFile->Error = D_GIF_ERR_NOT_ENOUGH_MEM;
      return GIF_ERROR;
    }
  ...
  if (GifFile->Image.ColorMap != NULL) {
    sp->ImageDesc.ColorMap = GifMakeMapObject(
        GifFile->Image.ColorMap->ColorCount, GifFile->Image.ColorMap->Colors);
...
int DGifGetLine(GifFileType *GifFile, GifPixelType *Line, int LineLen) {
  ...
  if (DGifDecompressLine(GifFile, Line, LineLen) == GIF_OK) {
    if (Private->PixelCount == 0) {
       ...
       */
      do
        if (DGifGetCodeNext(GifFile, &Dummy) == GIF_ERROR) return GIF_ERROR;
      while (Dummy != NULL);
    }
    return GIF_OK;
  } else
...
int DGifSavedExtensionToGCB(GifFileType *GifFile, int ImageIndex,
  ...
  if (ImageIndex < 0 || ImageIndex > GifFile->ImageCount - 1) return GIF_ERROR;
...
int DGifCloseFile(GifFileType *GifFile) {
  ...
  if (GifFile == NULL || GifFile->Private == NULL) return GIF_ERROR;
...
int DGifGetLZCodes(GifFileType *GifFile, int *Code) {
  ...
  if (*Code == Private->EOFCode) {
    ...
    do {
      if (DGifGetCodeNext(GifFile, &CodeBlock) == GIF_ERROR) return GIF_ERROR;
        ...
        if (sp->ImageDesc.Interlace) {
          ...
          for (i = 0; i < 4; i++)
            for (j = InterlacedOffset[i]; j < sp->ImageDesc.Height;
                 j += InterlacedJumps[i]) {
              if (DGifGetLine(GifFile, sp->RasterBits + j * sp->ImageDesc.Width,
