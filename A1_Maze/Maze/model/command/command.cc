...
void SolveCommand::Execute(std::vector<std::vector<int>> &data) {
  ...
  while (!queue.empty()) {
    ...
    if (left >= 0 && !(data[left / col][left % col] & Wall::kRight) &&
        !visited.count(left)) {
      queue.push(left);
      parent[left] = cur;
    }
    if (!(data[cur / col][cur % col] & Wall::kBot) && !visited.count(bot)) {
      queue.push(bot);
      parent[bot] = cur;
    }
    if (!(data[cur / col][cur % col] & Wall::kRight) && !visited.count(right)) {
      queue.push(right);
      parent[right] = cur;
    }
  }
