...
START_TEST(sub_test_4) {
  ...
  int answer = s21_sum_matrix(&A, &B, S21_NULL);
...
Suite *st_s21_sub_matrix(void) {
  Suite *s = suite_create("\033[5m\033[1;31m\033[47m-=SUB_MATRIX=-\033[0m");
  TCase *tc_sub_matrix = tcase_create("s21_sub_matrix_tc");

  tcase_add_test(tc_sub_matrix, sub_test_1);
  tcase_add_test(tc_sub_matrix, sub_test_2);
  tcase_add_test(tc_sub_matrix, sub_test_3);
  tcase_add_test(tc_sub_matrix, sub_test_4);
