...
s21::GraphView::~GraphView() {
  delete ui;
}

void s21::GraphView::SetValidators() {
  ui->line_xmin->setValidator(validateDouble_.get());
  ui->line_xmax->setValidator(validateDouble_.get());
  ui->line_ymin->setValidator(validateDouble_.get());
  ui->line_ymax->setValidator(validateDouble_.get());
}
...
void s21::GraphView::InitCalc() {
  ...
  contGraph_.ProcessGraph(result);
}
